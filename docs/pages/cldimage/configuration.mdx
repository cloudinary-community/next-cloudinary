...
<Table
  ...
  data={[
    ...
    },
    {
      ...
      type: 'boolean',
      default: '-',
      example: () => (<code>{`"space jellyfish"`}</code>),
...
> Note: For `aspectRatio` prop to work properly, `fill` prop must also be present alongside it, and `width` and `height` props must be absent.
...
[Learn more about angle](https://cloudinary.com/documentation/transformation_reference#ar_aspect_ratio) on the Cloudinary docs.
...
[Learn more about the Extract transformation](https://cloudinary.com/documentation/transformation_reference#e_extract) on the Cloudinary docs.
